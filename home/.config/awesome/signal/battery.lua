...
local function emit_battery_info()
    awful.spawn.easy_async_with_shell("cat /sys/class/power_supply/" .. batt .. "/capacity", function(out)
...
local battery_subscribe_script = "bash -c 'while (inotifywait -e modify /sys/class/power_supply/" .. batt .. "/capacity -qq) do echo; done'"
